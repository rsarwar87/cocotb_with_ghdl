...
----
  ...
  begin
   ...
   if (rising_edge(ps_lock_dly)) then
	...
	CLK0_GEN_P: process
	    variable ik0 : INTEGER := 0;
	    variable clk0_frac_rm_rl : REAL := 0.0;
	    variable clk0_frac_rm : INTEGER := 0;
	   	begin
	    if (rst_in_o = '1' or clk0ps_en = '0') then
	        clk0_cnt <=  0;
	        clk0_out <= '0';
	    elsif (clk0_nocnt = '1') then
	        clk0_out <= not clk0_out;
	    ...
	    elsif (clk0_frac_en = '1' and clk0in = '1') then
	        clk0_out <= '1';
	        clk0_frac_rm_rl := 0.0;
	        clk0_frac_rm := 0;
	        for ik0 in 1 to 7 loop
	            clk0_frac_rm_rl := clk0_frac_rm_rl - real (clk0_frac_rm);
				if (clk0_frac_rm_rl<0.00) then
					clk0_frac_rm_rl:=clk0_frac_rm_rl * (-1.000);
				else
					clk0_frac_rm_rl:= clk0_frac_rm_rl;
				end if;
	            clk0_frac_rm := real2int(clk0_frac_rm_rl);
	            wait for clk0_frac_ht +1 ps + clk0_frac_rm * 1 ps;
	            clk0_out <= '0';
	            clk0_frac_rm_rl := clk0_frac_rm_rl - real (clk0_frac_rm);
				if (clk0_frac_rm_rl<0.000) then
					clk0_frac_rm_rl:=clk0_frac_rm_rl* (-1.000);
				else
					clk0_frac_rm_rl:= clk0_frac_rm_rl;
				end if;
	            clk0_frac_rm := integer (clk0_frac_rm_rl);
	            wait for clk0_frac_lt +1 ps  + clk0_frac_rm * 1 ps;
	            clk0_out <= '1';
	        end loop;
	        wait for clk0_frac_ht +1 ps;
	        clk0_out <= '0';
	        wait for clk0_frac_lt +1 ps - period_vco1;
	    end if;
	   wait until rising_edge(rst) or (clk0in'event);
	end process;
